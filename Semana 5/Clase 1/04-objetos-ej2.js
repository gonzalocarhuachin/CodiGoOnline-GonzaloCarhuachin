...
//EJ1 ELABORAR UNA FUNCION QUE OBEDEZCA A LA SGTE ESTRUCTURA

//FUNCION QUE BUSCA ENTRE TODOS LOS PAISES EL NOMBRE DE UN PAIS DADO, CUANDO ENCUENTRA EL NOMBRE DEBERA IMPRIMIR LA CANTIDAD DE 
//POSITIVOS Y LA CANTIDAD DE FALLECIDOS, EN CASO DE QUE NO SE ENCUENTRE EL NOMBRE, LA FUNCION IMPRIMIRA ERROR EN EL NOMBRE
//PROCUREN USAR LA SENTENCIA BREAK AL ENCONTRAR EL NOMBRE
const buscarPorNombre = function (nombre) 
{
    for (let i = 0; i < resultados.locations.length; i++) 
    {
        if(resultados.locations[i].country === nombre)
        {
            ...
            console.log(`CASOS POSITIVOS: ${resultados.locations[i].latest.confirmed}`);
            console.log(`FALLECIDOS: ${resultados.locations[i].latest.deaths}`);
            break;
        }
        else
        {
...
buscarPorNombre("Peru");

//EJ2 FUNCION QUE RETORNA UN ARREGLO DE PAISES CUYA CANTIDAD DE INFECTADOS SUPERE EL 0.001% COMPARANDO LA CANTIDAD
//DE INFECTADOS VS LA POBLACION DE DICHO PAIS
...
const mostrarEstadisticas = function()
{
    ...
    for (let i = 0; i < resultados.locations.length; i++)
    {
        if(resultados.locations[i].latest.confirmed * 100 / resultados.locations[i].country_population > 0.001)
        {
            filtrados.push(resultados.locations[i].country);
        }
    }
    return filtrados;
}
